...
>
> 起点：本工程拷贝自 `E:\project`（已跑通的 Sobel 加速器），
> 原工程保持不动，作为可回滚的参照。
...
| # | 模块 | 输入 → 输出 | 实现 | 复用现有资产 | 状态 |
|---|---|---|---|---|---|
| 1 | `ov5640_capture` | DVP 16bit(8位数据) → AXIS 24bit RGB | **Verilog** | 无 | 待写 |
| 2 | `rgb2gray` | AXIS 24bit RGB → AXIS 8bit Y | HLS | 无 | 待写 |
| 3 | `gaussian_3x3` | AXIS 8bit → AXIS 8bit | HLS | **复用 `sobel_hls.cpp` 行缓存骨架** | 待写 |
| 4 | `sobel_accel` | AXIS 8bit → AXIS 8bit | HLS | ✅ **已有，直接用** | 完工 |
| 5 | `adaptive_thresh` | AXIS 8bit → AXIS 8bit | HLS | 无 | 待写 |
| 6 | `morph_close` | AXIS 8bit → AXIS 8bit | HLS | 复用 3×3 窗口骨架 | 待写 |
| 7 | `roi_extract` | AXIS 8bit → AXIS 8bit 96×96 | HLS | 无 | 待写 |
| 8 | `vdma_writer` | AXIS → DDR | **Xilinx VDMA IP** | BD 集成 | 待集成 |
| 9 | `hdmi_out` | DDR → HDMI 720p | Xilinx 视频 IP + TMDS | 无 | 待集成 |
...
|---|---|---|
| 需要精确控制时钟域/边沿 | ✅ DVP 采集 | ❌ |
| 需要例化原语（MMCM/BUFR） | （本项目已不需要，见下） | ❌ |
| 是"每像素一次运算"的规整流水 | ❌ | ✅ 灰度/滤波/阈值/形态学 |
| 需要频繁改参数调优 | ❌ | ✅ 阈值/增益/核大小 |

> **XCLK 生成已不需要**：原计划用 MMCM 给摄像头产生 24 MHz 主时钟。
> 2026-09-15 硬件方案改为**正点原子 ATK-OV5640**，该模块**板载
> 24 MHz 有源晶振**，不需要 PL 提供。这同时省下一根信号线
> （14 → 13 根），使全部信号能装进 Arduino 一个连接器。

**`ov5640_capture` 必须是 Verilog**：I2C/SCCB 状态机、VSYNC/HREF 边沿检测、
...
> （参照工程在 `E:\project`）。当前唯一的 BD 是 **`bd_video`**，
> 见 `vivado/README.md`。
...
| 项 | 状态 |
|---|---|
| 现有 Sobel 工程 | ✅ 综合/实现/时序收敛已跑通（WNS +1.100 ns） |
| 现有工程板级实测 | ❌ **未做**（板子未到） |
| 视频通路 BD | ❌ 未建 |
| OV5640 采集 | ❌ 未写 |
| PS 侧 CNN | ❌ 未开始（他人负责） |
| 引脚分配表 | ⚠ 基于原厂 XDC 推导，**接线前需万用表实测复核** |
...
- 引脚约束：`C:\Users\xiaomu\Desktop\prepare\PYNQ-Z2资料\PYNQ-Z2_v1.0_master.xdc`（TUL 原厂）
- 板卡与共享引脚：同目录 `PYNQ-Z2资料汇编.md`
- 现有工程接口与踩坑：`E:\project\README.md`（本工程内副本同名）
